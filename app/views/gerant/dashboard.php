<style>
  ...
  @media (min-width: 768px) {
    .grid-week { grid-template-columns: 140px repeat(<?= count($fenetreSemaines) ?>, 52px); }
    .cell { height:42px; font-size:13px; }
  }
</style>

<section class="p-4 md:p-8 md:pb-0 grid grid-cols-3 gap-2 md:gap-4">
  <div class="bg-gray-50 rounded-lg p-3 md:p-4 text-center">
    <div class="text-[11px] md:text-xs text-gray-500">Collecté</div>
    <div class="font-bold text-sm md:text-xl"><?= number_format($totalCollecte, 0, ',', ' ') ?></div>
    <div class="text-[10px] md:text-xs text-gray-400">FCFA</div>
  </div>
  <div class="bg-gray-50 rounded-lg p-3 md:p-4 text-center">
    <div class="text-[11px] md:text-xs text-gray-500">Apprenants</div>
    <div class="font-bold text-sm md:text-xl"><?= count($apprenants) ?></div>
  </div>
  <div class="bg-red-50 rounded-lg p-3 md:p-4 text-center">
    <div class="text-[11px] md:text-xs text-red-600">En retard</div>
    <div class="font-bold text-sm md:text-xl text-red-600"><?= $nbRetard ?></div>
  </div>
</section>

<div class="md:grid md:grid-cols-3 md:gap-8 md:px-8 md:mt-6">
  <section class="px-4 md:px-0 md:col-span-2">
    <h2 class="font-bold text-sm md:text-base mb-2">Tableau croisé - dernières semaines</h2>
  ...
  <aside class="px-4 md:px-0 mt-4 md:mt-0">
    <section>
      <h2 class="font-bold text-sm md:text-base mb-2">Campagnes actives</h2>
